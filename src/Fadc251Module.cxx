...
namespace Decoder {
  ...
  Module::TypeIter_t Fadc251Module::fgThisType =
    DoRegister( ModuleType( "Decoder::Fadc251Module" , 251 ));

  Fadc251Module::Fadc251Module(Int_t crate, Int_t slot)
    : VmeModule(crate, slot), fNumAInt(0), fNumTInt(0), fNumSample(0),
      fAdcData(0), fTdcData(0)
  {
    cout << "Fadc251Module::Fadc251Module()" << endl;
    Init();
  }

  Fadc251Module::~Fadc251Module() {
    delete [] fNumAInt;
    delete [] fNumTInt;
    delete [] fNumSample;
    delete [] fAdcData;
  ...
  void Fadc251Module::Init() {
    ...
    IsInit = kTRUE;
    fName = "FADC 251";
    SetMode(F251_SAMPLE);  // needs to be driven by cratemap ... later
  }


  Bool_t Fadc251Module::IsSlot(UInt_t rdata) {
#ifdef WITH_DEBUG
    if (fDebugFile) {
  ...
  void Fadc251Module::CheckSetMode() const {
    if (f251_setmode != F251_SAMPLE && f251_setmode != F251_INTEG) {
      cout << "Check the F251 setmode.  It is = "<<f251_setmode<<endl;
      cout << "And should be set to either "<<F251_SAMPLE<<"   or "<<F251_INTEG<<endl;
    }
  }

  Int_t Fadc251Module::GetMode() const {
#ifdef WITH_DEBUG
    if (fDebugFile)
      *fDebugFile << "GetMode ... "<<f251_setmode<< "   "<<f251_foundmode<<endl;
#endif
    if (f251_setmode == f251_foundmode) return f251_setmode;
    return -1;
  }

  void Fadc251Module::CheckFoundMode() const {
    if (f251_setmode != f251_foundmode) {
      cout << "Fadc251Module:: ERROR: The set mode "<<f251_setmode;
      cout << "   is not consistent with the found mode "<<f251_foundmode<<endl;
    }
  }

  Int_t Fadc251Module::GetAdcData(Int_t chan, Int_t ievent) const {
    Int_t result = GetData(GET_ADC, chan, ievent);
    if (result < 0) cout << "Fadc251Module:: WARNING:  Strange ADC data "<<endl;
    return result;
  }

  Int_t Fadc251Module::GetTdcData(Int_t chan, Int_t ievent) const {
    Int_t result = GetData(GET_TDC, chan, ievent);
    if (result < 0) cout << "Fadc251Module:: WARNING:  Strange TDC data "<<endl;
    return result;
  }

  Int_t Fadc251Module::GetNumSamples(Int_t chan) const {
    if (chan < 0 || chan > NADCCHAN) {
      cout << "ERROR:: Fadc251Module:: GetAdcData:: invalid channel "<<chan<<endl;
      return 0;
    }
    return fNumSample[chan];
  }

  Int_t Fadc251Module::GetData(Int_t which, Int_t chan, Int_t ievent) const {

    int index;
    int nevent;

    if ( !IsInit ) {
      cout << "ERROR:: Fadc251Module:: Not initialized "<<endl;
...
#ifdef WITH_DEBUG
    if (fDebugFile)
      *fDebugFile << "GetData:  f251_foundmode "<<f251_foundmode<<endl;
#endif
    if (f251_foundmode == F251_INTEG) {
      if (which == GET_ADC) {
...
#ifdef WITH_DEBUG
      if (fDebugFile) {
	*fDebugFile << "Fadc251:: info "<<which<<"   "<<f251_foundmode<<"   "<<f251_setmode<<endl;
	*fDebugFile << "ERROR:: Fadc251Module:: GetData:: invalid event "<<ievent<<"  "<<nevent<<endl;
      }
#endif
      return -1;
    }
    ...
    if (index < 0 || index > NADCCHAN*MAXDAT) {
      cout << "ERROR:: Fadc251Module:: GetData:: invalid index "<<index<<endl;
      return -1;
    }
  ...
  void Fadc251Module::Clear(const Option_t *opt) {
    f251_foundmode=-1;
    fNumEvents = 0;
    fNumTrig = 0;
    if (IsIntegMode()) {
  ...
  Int_t Fadc251Module::LoadSlot(THaSlotData *sldat, const UInt_t *evbuffer, const UInt_t *pstop) {
    // this increments evbuffer
#ifdef WITH_DEBUG
    if (fDebugFile)
      *fDebugFile << "Fadc251Module:: loadslot "<<endl;
#endif
    fWordsSeen = 0;
    Int_t numwords = 0;
    if (IsSlot(*evbuffer)) {
...
// This would not happen if CRL puts numwords into output
...
#ifdef WITH_DEBUG
    if (fDebugFile)
      *fDebugFile << "Fadc251Module:: num words "<<dec<<numwords<<endl;
#endif
...
#ifdef WITH_DEBUG
    if (fDebugFile)
      *fDebugFile << "Fadc251Module:: mode info "<<dec
		  <<f251_setmode<<"   "<<f251_foundmode<<endl;
#endif
    CheckFoundMode();
...
#ifdef WITH_DEBUG
      if (fDebugFile)
	*fDebugFile << "Fadc251:: Chan  "<<chan<<"  num sample "
...
#ifdef WITH_DEBUG
	if (fDebugFile)
	  *fDebugFile << "Fadc251:: indexing  "<<index<<"  "<<NADCCHAN*MAXDAT<<endl;
#endif
	if (index < 0 || index > NADCCHAN*MAXDAT) {
#ifdef WITH_DEBUG
	  if (fDebugFile)
	    *fDebugFile << "Fadc251:: INDEX problem !  "<<index<<endl;
#endif
	  cerr << "Fadc251:: WARN: index out of range "<<dec<<index<<endl;
	} else {
  ...
  Int_t Fadc251Module::Decode(const UInt_t *pdat)
  { // Routine from B. Moffit, adapted by R. Michaels for this class.
    ...
    if ((i_print==1) && (fDebugFile !=0))
      *fDebugFile << "Fadc251::Decode   "<< hex<< data<<"   "<<dec<<iword<<endl;
#endif
    iword++;
	...
	if( fadc_data.new_type )
	...
	else
	  {
...
#endif
	    f251_foundmode = F251_SAMPLE;
#ifdef WITH_DEBUG
	    if(  (i_print == 1) && (fDebugFile != 0) )
	      *fDebugFile << "Setting f251_foundmode "<<f251_foundmode<<endl;
#endif
	    chan=0;
	    if (fadc_data.chan < static_cast<UInt_t>(NADCCHAN)) {
	      chan = fadc_data.chan;
	    } else {
	      cout << "ERROR:: Fadc251Module:: ADC channel makes no sense !"<<endl;
	    }
		...
		cout << "Fadc251Module:: too many raw data words ? "<<endl;
	      }
	    }
#ifdef WITH_DEBUG
	    if(  (i_print == 1) && (fDebugFile != 0) )
	      *fDebugFile <<"Chan "<<chan<<"    Num samples "
			  <<fNumSample[chan]<<"   mode "<<f251_foundmode<<endl;
#endif
	  }
	break;
      case 5:		/* WINDOW SUM */
...
ClassImp(Decoder::Fadc251Module)
